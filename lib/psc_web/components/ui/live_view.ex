defmodule PscWeb.Components.UI.LiveView do
  @moduledoc """
  ...
  def handle_event("open_profile", _params, socket) do
    socket = PscWeb.Components.UI.LiveView.send_command(socket, "profile-sheet", "open")
    {:noreply, socket}
  end

  def handle_event("close_profile", _params, socket) do
    socket = PscWeb.Components.UI.LiveView.send_command(socket, "profile-sheet", "close")
    {:noreply, socket}
  end
  ```
  ...
  Use `PscWeb.Components.UI.JS.dispatch_command/3` when a LiveView JS command should control a
  component directly in the browser.

  ```heex
  <.button phx-click={%JS{} |> PscWeb.Components.UI.JS.dispatch_command("open", to: "#profile-sheet")}>
    Open
  </.button>
  ```
  ...
  ```elixir
  socket = PscWeb.Components.UI.LiveView.send_command(socket, "dialog", "open")
  ```

  With params:

  ```elixir
  socket = PscWeb.Components.UI.LiveView.send_command(socket, "chart", "update", %{series: data})
  ```
  """
...
defmodule PscWeb.Components.UI.JS do
  @moduledoc """
  ...
  commands sent by `PscWeb.Components.UI.LiveView.send_command/4`.

  ## Example

  ```heex
  <.button phx-click={%JS{} |> PscWeb.Components.UI.JS.dispatch_command("open", to: "#dialog")}>
    Open dialog
  </.button>
  ```
  ...
  <.button
    phx-click={
      %JS{}
      |> PscWeb.Components.UI.JS.dispatch_command("update", to: "#chart", detail: %{series: @series})
    }
  >
    Update chart
  </.button>
