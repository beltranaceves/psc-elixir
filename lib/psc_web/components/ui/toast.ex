defmodule PscWeb.Components.UI.Toast do
  @moduledoc """
  ...
  Use the helpers below from any `handle_event/3`:
      ...
      def handle_event("save", _params, socket) do
        {:noreply, PscWeb.Components.UI.Toast.toast_success(socket, "Saved!")}
      end
  ...
  Declare a `:template` next to the toaster and reference it by name:
      ...
      PscWeb.Components.UI.Toast.toast(socket, template: "new-follower")
  ...
  Per-toast, overriding the toaster-wide default for just this call:
      ...
      PscWeb.Components.UI.Toast.toast_success(socket, "Saved!",
  ...
  `!`-prefixed utilities there to reliably win the cascade:
      ...
      PscWeb.Components.UI.Toast.toast(socket, "Something broke",
  ...
  elements for `class` to style from scratch:
      ...
      PscWeb.Components.UI.Toast.toast(socket, "Bare toast", unstyled: true, class: "my-toast")

  `action`/`cancel` accept an optional `value` map, merged into the event
  payload pushed back to the LiveView (alongside `id`):

      PscWeb.Components.UI.Toast.toast(socket, "File deleted",
  ...
  * `:on-dismiss` - Fired whenever a toast is removed, with `%{id:, reason:}` in the payload.
  ...
  list/map of opts with no title:
      ...
      socket = PscWeb.Components.UI.Toast.toast(socket, "Saved!")
      socket = PscWeb.Components.UI.Toast.toast(socket, "Saved!", description: "Changes are live")
      socket = PscWeb.Components.UI.Toast.toast(socket, template: "new-follower", variant: "default")
  ...
  def toast(socket, toaster_id, message, opts) do
    PscWeb.Components.UI.LiveView.send_command(
      socket,
      toaster_id,
      "add",
  ...
  def toast_update(socket, id, opts) do
    ...
    PscWeb.Components.UI.LiveView.send_command(socket, "toaster", "update", params)
  end
  ...
  def toast_dismiss(socket, toaster_id \\ "toaster", id \\ nil) do
    ...
    PscWeb.Components.UI.LiveView.send_command(socket, toaster_id, "dismiss", params)
  end
